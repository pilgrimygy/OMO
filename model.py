...
class EnsembleDynamicsModel():
    def __init__(self, network_size, elite_size, state_size, action_size, reward_size=1, hidden_size=200, use_decay=False, jointly_lr=None):
        ...
        if jointly_lr is not None:
            self.jointly_optimizer = torch.optim.Adam(self.ensemble_model.parameters(), lr=jointly_lr)
    ...
    def evaluate(self, state, action, next_state, reward):
        ...
        ensemble_model_means, ensemble_model_vars = self.ensemble_model(inputs[None, :, :].repeat([self.network_size, 1, 1]), ret_log_var=False)

        ensemble_model_stds = ensemble_model_vars.sqrt()
        ...
        diff_state_dists = Normal(model_means, model_stds)

        diff_state = next_state - state

        inputs = torch.cat([reward.unsqueeze(-1), diff_state], -1)
