...
def readParser():
    parser = argparse.ArgumentParser(description='MOPO')
    ...
    parser.add_argument('--use_decay', type=bool, default=True, metavar='G',
                        help='whether to use exploration parameter decay (default: True)')
...
def train(args, env_sampler, predict_env, agent, env_pool, model_pool, writer):
    ...
    for epoch_step in range(args.num_epoch):
        ...
        for i in count():
            ...
            beta = max(0.1, 3 - 0.03 * epoch_step)
                    
            if len(env_pool) > args.min_pool_size:
                train_policy_steps += jointly_train_policy_model(args, total_step, train_policy_steps, env_pool, model_pool, agent, predict_env, beta, writer)

            total_step += 1

            if total_step % args.epoch_length == 0:
                eval_rewards = []
                while len(eval_rewards) < 10:
                ...
                writer.add_scalar('reward', eval_reward, total_step)

                logging.info("Step Reward: " + str(total_step) + " " + str(eval_reward))
                use_time = time.time() - start
                print(total_step, eval_reward, use_time)
...
def jointly_train_policy_model(args, total_step, train_step, env_pool, model_pool, agent, predict_env, beta, writer):
    ...
    for i in range(args.num_train_repeat):
        ...
        qf1_loss, qf2_loss, policy_loss, alpha_loss, _ = agent.update_parameters((batch_state, batch_action, batch_reward, batch_next_state, batch_done), args.policy_train_batch_size, i)

        env_state, env_action, env_reward, env_next_state, env_done = env_pool.sample(args.policy_train_batch_size)
    ...
    qf1_loss_step /= args.num_train_repeat
    qf2_loss_step /= args.num_train_repeat
    policy_loss_step /= args.num_train_repeat
    alpha_loss_step /= args.num_train_repeat
    model_j_loss_step /= args.num_train_repeat
    model_d_loss_step /= args.num_train_repeat

    if total_step % 200 == 0:
        writer.add_scalar('q1loss', qf1_loss, total_step)
        writer.add_scalar('q2loss', qf2_loss, total_step)
        writer.add_scalar('policyloss', policy_loss, total_step)
        writer.add_scalar('alphaloss', alpha_loss, total_step)
        writer.add_scalar('model_j_loss', model_j_loss_step, total_step)
        writer.add_scalar('model_d_loss', model_d_loss_step, total_step)
...
def main(args=None):
    ...
    if args.model_type == 'pytorch':
        env_model = EnsembleDynamicsModel(args.num_networks, args.num_elites, state_size, action_size, args.reward_size, args.pred_hidden_size, use_decay=args.use_decay, jointly_lr=3e-7)
    else:
    ...
    # Writer
    writer = SummaryWriter(log_dir="./results/log/MOPO-"+nowtime)
