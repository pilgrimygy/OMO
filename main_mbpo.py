...
def train(args, env_sampler, eval_env_sampler, predict_env, agent, env_pool, model_pool, writer):
    total_step = 0
    rollout_length = 1
    ...
    for epoch_step in range(args.num_epoch):
        ...
        for i in count():
            ...
            if len(env_pool) > args.min_pool_size:
                train_policy_steps += train_policy_repeats(args, total_step, train_policy_steps, cur_step, env_pool, model_pool, agent, writer)
...
def train_policy_repeats(args, total_step, train_step, cur_step, env_pool, model_pool, agent, writer):
    ...
    for i in range(args.num_train_repeat):
        ...
        writer.add_scalar('qloss', qf1_loss + qf2_loss, total_step)
...
def main(args=None):
    ...
    if args.model_type == 'pytorch':
        env_model = EnsembleDynamicsModel(args.num_networks, args.num_elites, state_size, action_size, args.reward_size, args.pred_hidden_size, use_decay=args.use_decay)
    else:
    ...
    writer = SummaryWriter(log_dir="./results/log", filename_suffix="-MBPO-"+nowtime)

    train(args, env_sampler, eval_env_sampler, predict_env, agent, env_pool, model_pool, writer)
